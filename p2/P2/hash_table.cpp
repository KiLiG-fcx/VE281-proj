//
// Created by lmyannie60 on 2020/11/8.
//
...
public:
    ...
    private:
        ...
        void increment() {
            ...
            ++newListItBefore; // to the first one of new list
...
protected:                                                                  // DO NOT USE private HERE!
    ...
    size_t findMinimumBucketSize(size_t bucketSize) const {
        ...
        size_t rq2=tableSize/maxLoadFactor;
       // int record=0;
    ...
    Value &ins(Iterator ite, const Key &k, const Value &v){
        this->tableSize++;
        ite.bucketIt->emplace_after(ite.listItBefore,k,v); // insert into the forward_list
        firstBucketIt = std::min(firstBucketIt, ite.bucketIt); //update the first_bucket_it
        if(loadFactor()>this->maxLoadFactor) {
            this->rehash(buckets.size());
        }
        Value &value=ite->second;
        return value;
    }

    Iterator ers(Iterator ite){
        this->tableSize--;
        ite.bucketIt->erase_after(ite.listItBefore);
        Iterator itt=ite;
        itt++;
        if(ite.bucketIt==firstBucketIt && ite.bucketIt->empty()){
...
public:
    HashTable() : //initialize para. of hashtable (6)
    ...
    HashTable(const HashTable &that) { //copy a hash table
        // TODO: implement this function
     ...
     */
    Iterator find(const Key &key) { //find the key in this
        // TODO: implement this function
        ...
        for(buck_first++;buck_first!=it.bucketIt->end();++buck_first){
            if(KeyEqual(key,buck_first->first)){ // find the key in this bucket
                return it;
            }
            it.listItBefore++;
        }
    ...
    bool insert(const Iterator &it, const Key &key, const Value &value) {
        ...
        if(itl!=it.bucketIt->end() && keyEqual(key,itl->first)){ //find the key in the table
            itl->second=value; //update the value
            return false; //already exists, return f
        }
        ins(it,key,value); // not exist, add the value
        return true;
    }
    ...
    void rehash(size_t bucketSize) {
        ...
        // TODO: implement this function
        HashTableData htd(bucketSize);
        for(auto &item:buckets){
            for(auto &elem:item){
                auto nit=hashKey(elem.first,bucketSize);
                auto &nvalue=htd[nit];
                nvalue.emplace_front(std::move(elem));
            }
        }
        buckets.swap(htd);
        firstBucketIt=buckets.end();
...
int main(){
    HashTable<int,int,std::hash<int>,std::equal_to<int>> ht(11);
