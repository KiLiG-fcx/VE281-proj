...
protected:                                                                  // DO NOT USE private HERE!
    ...
    size_t findMinimumBucketSize(size_t bucketSize) const {
        ...
        bucketSize = std::max(bucketSize, (size_t) (tableSize / maxLoadFactor) + 1);
        auto maxIndex = HashPrime::num_distinct_sizes;
        if (bucketSize > HashPrime::g_a_sizes[maxIndex - 1]) {
            throw std::range_error("hash table too large!");
        }
        bucketSize = *(std::lower_bound(HashPrime::g_a_sizes, HashPrime::g_a_sizes + maxIndex, bucketSize));
        return bucketSize;
    }

    // TODO: define your helper functions here if necessary
    Value &insertHelper(Iterator it, const Key &key, const Value &value) {
        if (it.bucketIt == buckets.end()) {
            throw std::runtime_error("insert to end!");
        }
        ++tableSize;
        it.bucketIt->emplace_after(it.listItBefore, key, value);
        firstBucketIt = std::min(firstBucketIt, it.bucketIt);
        auto &temp = it->second;
    ...
    Iterator eraseHelper(Iterator it) {
        if (it.bucketIt == buckets.end()) {
            throw std::runtime_error("erase from end!");
        }
        --tableSize;
        auto newIt = it;
        ++newIt;
...
public:
    ...
    Iterator find(const Key &key) {
        ...
        for (++listIt; listIt != it.bucketIt->end(); ++listIt) {
            if (keyEqual(key, listIt->first)) {
                return it;
            }
            ++it.listItBefore;
        }
    ...
    bool insert(const Iterator &it, const Key &key, const Value &value) {
        ...
        auto listIt = it.listItBefore;
        ++listIt;
        if (listIt != it.bucketIt->end() && keyEqual(listIt->first, key)) {
            listIt->second = value;
            return false;
        }
        insertHelper(it, key, value);
        return true;
    }
    ...
    Value &operator[](const Key &key) {
        ...
        return insertHelper(it, key, Value());
    }
    ...
    void rehash(size_t bucketSize) {
        ...
        for (auto &list : buckets) {
            for (auto &p : list) {
                auto index = hashKey(p.first, bucketSize);
                auto &newList = newBuckets[index];
                newList.emplace_front(std::move(p));
            }
        }
